...
class SSDHead(nn.Module):
    ...
    Args:
        ...
        anchor_strides (Iterable): Anchor strides. 1 pixel's real size on the feature map.  (8, 16, 32, 64, 100, 300).
        basesize_ratio_range (Iterable): Anchor base scales at different resolutions. (0.1, 0.9). Scales are [0, 1] scaled.
    ...
    def __init__(
        ...
        for k in range(len(self.anchor_strides)):
            ...
            # filter the anchors. Some are discarded according to the SSD paper.
    ...
    def loss(
        ...
        """
        Args:
            cls_scores: Number_of_levels x Number_of_images x num_classes x H x W
            bbox_preds: Number_of_levels x B x num_anchors*4 x H x W
