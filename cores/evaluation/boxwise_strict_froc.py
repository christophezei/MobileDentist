...
[
    ...
    # each class
        ...
        # each image
            ...
            # 1 x N. 1 for 1 if matched, 0 is not matched.
            image_wise_match = np.sum(image_wise_match, axis=0, keepdims=False)
            # number_of_images x N.
            cat_wise_match.append(image_wise_match)
            # number_of_images x N.
...
def plot(maxiou_match, maxiou_confidence, num_class, num_images):
    ...
    for i in range(num_class):
        ...
        plt.ylim(0, 1)
        plt.plot(avg_fp, tpr)
        plt.savefig(dir_path + '/visualization/boxwise_strict_froc_{}.png'.format(i))
    ...
    plt.savefig(dir_path + '/visualization/boxwise_strict_froc_all.png')


def plot_boxwise_strict_froc(
        results,
        truths,
        threshold,
        num_class,
...
[
    ...
    plot(maxiou_match, maxiou_confidence, num_class, num_images=len(truths))
