...
from cores.bbox.geometry import bbox_overlaps
from cores.bbox.AssignResult import AssignResult
...
class MaxIoUAssigner(object):
    ...
    assigned gt. The assignment is done in following steps, the order matters:
    ...
    def assign(self, bboxes, gt_bboxes, gt_labels=None):
        ...
        Returns:
            num_gts, assigned_gt_inds, max_overlaps, assigned_labels
        """
        if bboxes.shape[0] == 0 or gt_bboxes.shape[0] == 0:
            raise ValueError('No gt or bboxes')
        bboxes = bboxes[:, :4]
        # [Number_of_gt_bboxes, Number_of_bboxes]
        overlaps = bbox_overlaps(gt_bboxes, bboxes, mode='iou', is_aligned=False)
